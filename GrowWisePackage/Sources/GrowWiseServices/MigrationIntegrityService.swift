...
public final class MigrationIntegrityService {
    ...
    public enum IntegrityError: LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case let .backupFailed(reason):
                "Backup creation failed: \(reason)"

            case let .verificationFailed(reason):
                "Migration verification failed: \(reason)"

            case let .rollbackFailed(reason):
                "Migration rollback failed: \(reason)"

            case let .migrationAborted(reason):
                "Migration aborted: \(reason)"

            case let .progressTrackingFailed(reason):
                "Progress tracking failed: \(reason)"

            case let .invalidState(reason):
                "Invalid migration state: \(reason)"

            case .partialMigrationCorrupted:
                "Partial migration data is corrupted and cannot be recovered"

            case let .dryRunFailed(reason):
                "Dry run test failed: \(reason)"
            }
        }
    }
    ...
    public enum MigrationStatus: String, Codable {
        ...
        public var description: String {
            switch self {
            case .notStarted: "Migration not started"
            case .backupInProgress: "Creating backup"
            case .backupCompleted: "Backup completed"
            case .migrationInProgress: "Migration in progress"
            case .verificationInProgress: "Verifying migration"
            case .completed: "Migration completed successfully"
            case .failed: "Migration failed"
            case .rolledBack: "Migration rolled back"
            case .partiallyComplete: "Migration partially complete"
            }
        }
    }
    ...
    public struct MigrationProgress: Codable {
        ...
        public var canResume: Bool {
            status == .partiallyComplete || status == .failed
        }
    }
    ...
    public struct DataChecksum {
        ...
        public var isValid: Bool {
            originalHash == migratedHash && verified
        }
    }
    ...
    public func getMigrationStatus(sessionId: String) -> MigrationProgress? {
        try? loadProgress(sessionId: sessionId)
    }
    ...
    public func performDryRun(keys: [String]) throws -> MigrationReport {
        try performSecureMigration(keys: keys, dryRun: true)
    }
