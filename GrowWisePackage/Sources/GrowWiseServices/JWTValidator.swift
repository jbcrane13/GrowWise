...
public final class JWTValidator {
    ...
    public enum JWTValidationError: LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case .invalidFormat:
                "JWT format is invalid - must have three parts separated by dots"

            case .invalidHeader:
                "JWT header is invalid or cannot be decoded"

            case .invalidPayload:
                "JWT payload is invalid or cannot be decoded"
            ...
            case let .invalidIssuer(expected, actual):
                "Invalid issuer - expected: \(expected), actual: \(actual ?? "null")"

            case let .invalidAudience(expected, actual):
                "Invalid audience - expected: \(expected), actual: \(actual ?? "null")"

            case let .missingClaim(claim):
                "Missing required claim: \(claim)"

            case .invalidPublicKey:
                "Invalid or missing public key for RS256 verification"

            case .invalidSharedSecret:
                "Invalid or missing shared secret for HS256 verification"
            }
        }
    }
    ...
    public struct JWTPayload: Codable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            for key in dynamicContainer.allKeys {
                let standardKeys = CodingKeys.allCases.map(\.rawValue)
