...
public final class EncryptionService: @unchecked Sendable {
    ...
    public enum EncryptionError: LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .migrationFailed:
                "Failed to migrate encryption keys"
            ...
            case .complianceViolation:
                "Operation violates security compliance requirements"

            case .unsupportedKeyVersion:
                "Unsupported key version for decryption"
            }
        }
    }
    ...
    public init(storage: KeychainStorageService) {
        ...
        useLegacyFallback = !SecureEnclaveKeyManager.isSecureEnclaveAvailable
    ...
    public var hasEncryptionKey: Bool {
        if useLegacyFallback {
            ...
            keyRotationManager.currentKeyVersion > 0 ||
    ...
    public func generateComplianceReport(period: DateInterval) async -> KeyRotationManager.ComplianceReport {
        await keyRotationManager.generateComplianceReport(period: period)
    }
    ...
    public func getAuditTrail(from startDate: Date, to endDate: Date) -> [KeyRotationManager.AuditEvent] {
        keyRotationManager.getAuditTrail(from: startDate, to: endDate)
    }
    ...
    public func getRotationPolicy() -> KeyRotationManager.RotationPolicy {
        ...
        KeyRotationManager.RotationPolicy.defaultPolicy()
    }
    ...
    public func getActiveKeyVersions() -> [Int] {
        keyRotationManager.getActiveKeyVersions()
    }

    /// Get current key version
    public var currentKeyVersion: Int {
        keyRotationManager.currentKeyVersion
    }
    ...
    public func migrateLegacyEncryption() throws {
        ...
        _ = try secureEnclaveKeyManager.getSymmetricKey()
    ...
    public func getMigrationInfo() -> MigrationInfo {
        migrationService.getMigrationInfo()
    }
    ...
    public var isUsingSecureEnclave: Bool {
        !useLegacyFallback && secureEnclaveKeyManager.hasSecureEnclaveKey()
    }
    ...
    private func getCurrentEncryptionKey() throws -> SymmetricKey {
        if useLegacyFallback {
            ...
            try migrationService.getLegacyKey()
        } else {
            // Use key rotation manager (preferred)
            try keyRotationManager.getCurrentEncryptionKey()
        }
    }
...
public struct KeyRotationStatus {
    ...
    public var description: String {
        if rotationOverdue {
            "Key rotation overdue (Compliance violation)"
...
public struct SecurityStatus {
    ...
    public var complianceStatus: String {
        if keyRotationStatus.rotationOverdue {
            "Non-compliant: Key rotation overdue"
        } else if keyRotationStatus.rotationNeeded {
            "Warning: Key rotation due soon"
        } else if isUsingSecureEnclave {
            "Fully compliant: Secure Enclave + Key rotation"
        } else {
            "Partially compliant: Consider upgrading to Secure Enclave"
        }
    }
}
