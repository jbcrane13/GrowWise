...
public final class LegacyEncryptionMigrationService {
    ...
    public enum MigrationError: LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case .legacyKeyNotFound:
                "Legacy encryption key not found"

            case .migrationFailed:
                "Failed to migrate encryption data"

            case .decryptionFailed:
                "Failed to decrypt legacy data"

            case .reencryptionFailed:
                "Failed to re-encrypt with new key"

            case .keyRetrievalFailed:
                "Failed to retrieve legacy encryption key"
            }
        }
    }
    ...
    public var hasLegacyKey: Bool {
        keychainStorage.exists(for: legacyKeyIdentifier)
    }
    ...
    public func migrateEncryptedData(
        ...
        let plaintext: Data = if let authData = authenticatedData {
            try decryptLegacyData(legacyEncryptedData, authenticatedData: authData)
        } else {
            try decryptLegacyData(legacyEncryptedData)
        }

        // Re-encrypt with new key
        do {
            let sealedBox: AES.GCM.SealedBox = if let authData = authenticatedData {
                try AES.GCM.seal(plaintext, using: newKey, authenticating: authData)
            } else {
                try AES.GCM.seal(plaintext, using: newKey)
            }
...
public enum MigrationAction {
    ...
    public var description: String {
        switch self {
        case .noActionNeeded:
            "No migration needed - using modern encryption"

        case .migrateToSecureEnclave:
            "Migrate to Secure Enclave for enhanced security"

        case .keepLegacyWithWarning:
            "Keep legacy encryption (Secure Enclave not available)"
        }
    }
}
