...
public final class KeychainManager: KeychainStorageProtocol, @unchecked Sendable {
    ...
    private lazy var rateLimiter: RateLimiter = RateLimiterFactory.create(with: self)
    ...
    private init() {
        // Initialize services
        storageService = KeychainStorageService(service: service, accessGroup: accessGroup)
        encryptionService = EncryptionService(storage: storageService)
        tokenService = TokenManagementService(encryptionService: encryptionService, storage: storageService)
        dataTransformationService = DataTransformationService(storage: storageService, encryptionService: encryptionService)
        migrationIntegrityService = MigrationIntegrityService(keychainStorage: storageService)
    ...
    private func mapServiceError(_ error: Error) -> KeychainError {
        if let transformationError = error as? DataTransformationService.TransformationError {
            ...
            case let .storageError(underlyingError):
    ...
    private func mapRateLimiterError(_ error: RateLimiter.RateLimiterError) -> KeychainError {
        ...
        case let .accountLocked(unlockAt):
            .accountLocked(unlockAt: unlockAt)

        case let .storageError(underlyingError):
            .serviceError(underlyingError)

        case let .invalidKey(reason):
            .invalidKey(reason)

        case let .configurationError(reason):
            .serviceError(NSError(domain: "RateLimiterConfiguration", code: -1, userInfo: [NSLocalizedDescriptionKey: reason]))
        }
    }
    ...
    public func performDryRunMigration() throws -> MigrationIntegrityService.MigrationReport {
        try migrateFromUserDefaults(dryRun: true)
    }
    ...
    public func getMigrationStatus(sessionId: String) -> MigrationIntegrityService.MigrationProgress? {
        migrationIntegrityService.getMigrationStatus(sessionId: sessionId)
    }
    ...
    public func credentialsNeedRefresh() -> Bool {
        tokenService.credentialsNeedRefresh()
    }
    ...
    public func checkAuthenticationRateLimit(
        ...
        rateLimiter.checkLimit(for: identifier, operation: operation)
    }
    ...
    public func retrieveSecureCredentialsWithBiometric(
        ...
        await auditLogger.logAuthentication(
            ...
            details: [
                "biometric_type": getBiometricTypeString(),
                "reason": reason,
        ...
        do {
            ...
            await auditLogger.logCredentialOperation(
                type: .credentialAccess,
                userId: identifier,
                result: .success,
...
extension KeychainManager {
    ...
    public func storeCodableWithBiometricProtection(_ object: some Codable, for key: String) throws {
