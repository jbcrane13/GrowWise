...
public final class SecureEnclaveKeyManager: @unchecked Sendable {
    ...
    public enum SecureEnclaveError: LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .keyDerivationFailed:
                "Failed to derive symmetric key from Secure Enclave key"

            case .keyNotFound:
                "Secure Enclave key not found"

            case .keyAccessFailed:
                "Failed to access Secure Enclave key"
    ...
    public func getSymmetricKey() throws -> SymmetricKey {
        ...
        let derivedKey: SymmetricKey = if hasSecureEnclaveKey() {
            try deriveSymmetricKeyFromExistingKey()
        } else {
            try generateNewKeyAndDeriveSymmetric()
        }
...
extension LAContext {
    fileprivate static func createContext() -> LAContext {
