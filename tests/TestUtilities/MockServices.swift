...
public final class MockDataService {
    ...
    public func getCurrentUser() -> User? {
        users.last // Return most recently created user
    }
    ...
    public func fetchPlants(for garden: Garden? = nil) -> [Plant] {
        ...
        return plants.filter(\.isUserPlant)
    }

    public func fetchPlantDatabase() -> [Plant] {
        plants.filter { !$0.isUserPlant }
    }
    ...
    public func getGardeningStats() -> GardeningStats {
        let totalPlants = plants.count(where: { $0.isUserPlant })
        let healthyPlants = plants.count(where: { $0.isUserPlant && $0.healthStatus == .healthy })
